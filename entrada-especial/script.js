...
function render() {
  ...
  if (!value) status.textContent = 'Digite uma data para continuar';
    ...
    status.textContent = 'Essa não é a data. Tente novamente';
  }
}
...
function unlockBook() {
  if (unlocked) return;
  unlocked = true;
  status.textContent = 'Data confirmada. Agora você pode prosseguir.';
  continueArea.classList.add('ready');
  continueArea.setAttribute('aria-hidden', 'false');
  clearTimeout(toastTimer);
  toast.classList.add('visible');
  toast.setAttribute('aria-hidden', 'false');
  toastTimer = setTimeout(() => { toast.classList.remove('visible'); toast.setAttribute('aria-hidden', 'true'); }, 2200);
}
...
openBook.addEventListener('click', () => {
  if (unlocked) window.location.href = 'memories.html';
});
render();
